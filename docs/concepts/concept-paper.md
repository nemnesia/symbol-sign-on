...
本設計では、**Symbolブロックチェーンの使用を必要最小限に留める**ことを明確な設計思想としています。

**背景と理由:**

- ブロックチェーンのストレージ容量は有限であり、全ノードが全トランザクションを保持する必要がある
- 不要なトランザクションの蓄積は、ノード運用者の負担増加とネットワーク全体の持続可能性を損なう
- 認証の度にオンチェーントランザクションを発行すると、トラフィック増大により手数料上昇やノード負荷の問題が生じる

**具体的な役割分担:**

| 処理内容       | 実行場所                       | 理由                                                         |
| -------------- | ------------------------------ | ------------------------------------------------------------ |
| 認証署名の検証 | Auth Server（オフチェーン）    | 毎回の認証をオンチェーン化すると膨大なトランザクションが発生 |
| セッション管理 | Auth Server（オフチェーン）    | ログイン状態の管理は頻繁に変化するため不適切                 |
| キーリンク設定 | Symbolチェーン（オンチェーン） | 鍵の有効性を公開検証可能にするため必須                       |
| キーリンク更新 | Symbolチェーン（オンチェーン） | 鍵の変更履歴を改ざん不可能にするため必須                     |
| マルチシグ設定 | Symbolチェーン（オンチェーン） | 権限構造を検証可能にするため必須                             |

この設計により、ユーザーが日常的にログインを繰り返しても、オンチェーン書き込みは「鍵の変更時のみ」に限定されます。これにより、Symbolチェーンの持続可能な運用と、多数のユーザーを支える認証基盤の両立を実現します。

> **補足:**  
> 認証に必要な「読み取り」（鍵状態の参照）はノードへの負荷が軽微であり、REST API を通じて効率的に実行できます。書き込み（トランザクション発行）のみを最小限に抑えることで、エコシステム全体の健全性を保ちます。
