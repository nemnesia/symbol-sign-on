...
Symbol Sign-On は DID Core の全要素を忠実に再現することを目的とした設計ではありません。一般ユーザー向けの認証基盤として成立させるため、以下の点を意図的に簡略化・省略しています。

- DID Document の明示的な定義
- DID Resolver / Universal Resolver の実装
- 認証以外（VC、Service Discovery 等）の用途別鍵管理

これらは分散アイデンティティの表現力を高めますが、ログイン用途においては UX、実装コスト、運用負荷を著しく増加させる要因となります。本設計は「フル DID」ではないことを明示しつつも、以下の中核的な考え方は重視しています。
